...
int main()
{
    ...
    if (menu_option == 3) { // Handle menu option of 3 to exit
        ...
        string leaveString = "*";
        int numOfSpaces = (2 * number_of_tree_layers) + 1;
        int leavesInLayer = 1;

        for(int treeLayer = 1; treeLayer <= number_of_tree_layers; treeLayer++) {
            for (int leaveLayer = 0; leaveLayer < 3; leaveLayer++) {
                cout << setw(numOfSpaces);
        ...
        for(int treeLayer = 1; treeLayer <= number_of_tree_layers; treeLayer++) {
            cout << setw(leavesInLayer + 1) << "|||" << endl;
            cout << setw(leavesInLayer + 1) << "|||" << endl;
        }
